...
RSpec.describe "Conformance: work-mods.xml projection" do
  ...
  it "reproduces the mods-gem name display_value_w_date (quirks included)" do
    expect(without_display_attributes(projection[:names])).to eq(
      [
  ...
  it "projects the remaining scalar/array fields" do
    aggregate_failures do
      expect(without_display_attributes(projection[:languages])).to eq([{ term: "English", object_part: nil,
                                                                          script: nil }])
      ...
      expect(without_display_attributes(projection[:identifiers]))
